...
def main_method(fullpath=None, outputdir=None):
    ...
    outputdirname = os.path.join(os.curdir, outputdir)
    ...
    for i in range(0, len(to_write)):
        write_name = os.path.join(outputdirname, (outputfilename if i == 0 else outputfilename + '_' + str(i)) + '.json')
    ...
    write_txt = os.path.join(outputdirname, outputfilename + '.txt')
...
def final_json(j_dict):
    ...
    with open(os.path.join(cdir,'Assets','header.json'), 'r') as f:
        header = json.load(f)
    res = []
...
def param_names():
    ...
    with open('Assets/params.csv') as csv_table:
        for row in csv.DictReader(csv_table):
            print(row)
    with open('Assets/params.csv') as csv_table:
        return {row['Short name']: row['Full name'] for row in csv.DictReader(csv_table)}
